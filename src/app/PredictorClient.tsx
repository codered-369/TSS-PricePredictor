...
export default function Dashboard({ initialData }: { initialData: any[] }) {
  ...
  const getInitialSelectedDate = () => {
    if (initialData && initialData.length > 0) {
      const dStr = initialData[initialData.length - 1].d;
  ...
  const [selectedDate, setSelectedDate] = useState(getInitialSelectedDate()); // Format: YYYY-MM-DD for the input
  ...
  useEffect(() => {
    // We already have initialData from SSR, but we still run loadData to check if
    // a background sync is needed after 3 PM.
    loadData();
    // Fetch visitor count
    fetch('/api/visitors')
      .then(res => res.json())
